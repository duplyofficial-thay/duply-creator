"""
context_builder.py — assembles the LLM context for tawan.chat.reply.
...
Edit this file to customize how ตะวัน uses user data.
Persona character → edit public.duply_duples.persona in Supabase.
Operational config → edit tawan_ai.agent_profiles.system_prompt in Supabase.
"""
...
# 4 levels up: reply/ → chat/ → tawan/ → duples/ → repo root (duply-agents/)
...
_CARD_DIR = os.path.join(_DIR, "..", "..", "card")
if _CARD_DIR not in sys.path:
    sys.path.insert(0, _CARD_DIR)

from card_config import REPLY_OUTPUT_PROMPT  # noqa: E402
...
def _build_system_prompt(agent_blocks: dict, persona: str) -> str:
    ...
    parts = [
        ...
        _section("Examples", p.get("examples")),
