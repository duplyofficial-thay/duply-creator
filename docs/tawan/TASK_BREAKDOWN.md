...
| TWN-0406 | Implement structured Customer Memory with source and confidence | Task | Standard | Data | Critical | Review | TWN-0313 | Local policy validates source and confidence and rejects unverified inferred confirmation; persistence and expiry jobs remain |
...
| TWN-0503 | Implement initial Task types and deterministic deduplication | Task | Standard | Platform | High | Review | TWN-0502 | Local policy derives stable action idempotency keys; runtime task persistence and retry integration remain |
...
| TWN-0505 | Implement unauthorized-discount escalation without promise | Task | Standard | Platform | High | Review | TWN-0503, TWN-0504 | Capture validation rejects unsafe model writes; runtime waiting-owner Task wiring remains |
...
| TWN-0507 | Test lifecycle authorization, retries, races, timeout, and revoked access | Review | Shared | Security | Critical | Review | TWN-0501 through TWN-0506 | Local policy tests cover authority, retries/idempotency, takeover, and unsafe transitions; database race tests remain |
| TWN-0508 | Implement store-defined Customer Tier rules and evidence | Task | Standard | Data | High | Review | TWN-0406, TWN-0501 | Local policy recommends the highest matching store-local tier with explainable order/value evidence; persistence, dates, expiry, and owner review remain |
...
| TWN-0602 | Implement deterministic price precedence and auditable exceptions | Task | Standard | Platform | Critical | Review | TWN-0306, TWN-0504, TWN-0509 | Local policy enforces customer, Pro Campaign, tier, quantity/wholesale, then standard precedence; persistence and audit wiring remain |
| TWN-0603 | Implement Order lifecycle and immutable confirmed line snapshots | Task | Standard | Platform | Critical | Review | TWN-0501, TWN-0601, TWN-0602 | Local policy rejects invalid transitions and non-owner payment completion; database transaction implementation remains |
| TWN-0604 | Implement stock reservation, configurable expiry, and release | Task | Standard | Data | Critical | Review | TWN-0601, TWN-0603 | Local policy rejects negative quantities and oversell; atomic database reservation and expiry job remain |
...
| TWN-0606 | Implement payment extraction and duplicate-evidence blocking | Task | Standard | Security | Critical | Review | TWN-0605 | Local policy blocks matching hash/fingerprint/reference before approval; provider extraction and cross-store platform review remain |
...
| TWN-0615 | Implement response planning and deterministic command validation | Task | Standard | Platform | Critical | Review | TWN-0504, TWN-0603, TWN-0614 | Local policy validates capture command types, idempotency, permanent-knowledge approval, and paid-order authority; runtime response-plan adapter remains |
